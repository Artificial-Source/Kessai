...
impl SettingsService {
    ...
    pub fn get(&self) -> Result<Settings> {
        ...
        let result = conn.query_row(
            "SELECT id, theme, currency, display_exchange_rates, notification_enabled, notification_days_before, monthly_budget,
            ...
            [],
            |row| {
                let display_exchange_rates_str: String = row.get(3).unwrap_or_else(|_| "{}".to_string());
                let display_exchange_rates = serde_json::from_str(&display_exchange_rates_str)
                    .unwrap_or_default();
                let notification_days_str: String = row.get(5)?;
                ...
                let animation_speed_str: String = row.get(10)?;
                Ok(Settings {
                    id: row.get(0)?,
                    ...
                    notification_advance_days: row.get::<_, i32>(11).unwrap_or(1),
                    notification_time: row.get::<_, String>(12).unwrap_or_else(|_| "09:00".to_string()),
                    monthly_budget: row.get(6)?,
                    reduce_motion: row.get::<_, i32>(7)? != 0,
                    enable_transitions: row.get::<_, i32>(8)? != 0,
                    enable_hover_effects: row.get::<_, i32>(9)? != 0,
    ...
    pub fn update(&self, data: UpdateSettings) -> Result<Settings> {
        ...
        if let Some(ref display_exchange_rates) = data.display_exchange_rates {
            sets.push("display_exchange_rates = ?");
            values.push(Box::new(serde_json::to_string(display_exchange_rates)?));
        }

...
const MIGRATIONS: &[Migration] = &[
    ...
    Migration {
        version: 16,
        description: "add_display_exchange_rates_to_settings",
        sql: r#"
            ALTER TABLE settings ADD COLUMN display_exchange_rates TEXT NOT NULL DEFAULT '{}';
        "#,
    },
];
...
///
/// Creates a `_kessai_migrations` tracking table if it doesn't exist,
/// then applies any migrations whose version hasn't been recorded yet.
pub fn run_migrations(conn: &Connection) -> Result<()> {
    conn.execute_batch(
        "CREATE TABLE IF NOT EXISTS _kessai_migrations (
    ...
    let applied: Vec<u32> = {
        let mut stmt = conn.prepare("SELECT version FROM _kessai_migrations ORDER BY version")?;
    ...
    for migration in MIGRATIONS {
        ...
        if should_skip_migration(conn, migration.version) {
            ...
            conn.execute(
                "INSERT INTO _kessai_migrations (version, description) VALUES (?1, ?2)",
...
mod tests {
    ...
    fn test_migrations_run_on_fresh_db() {
        ...
        assert!(table_exists(&conn, "_kessai_migrations"));
    ...
    fn test_migrations_are_idempotent() {
        ...
        let count: i64 = conn
            .query_row("SELECT COUNT(*) FROM _kessai_migrations", [], |r| r.get(0))
            .unwrap();
        assert_eq!(count, 16);
    }
}

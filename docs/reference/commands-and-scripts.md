...
| `pnpm test:e2e:web-smoke` | `playwright test -c playwright.web-smoke.config.ts e2e/web-smoke.spec.ts` | CI-style browser smoke run against the real web backend on port `3001` |
...
| `scripts/release-dry-run.js` | Safe release dry-run wrapper | Snapshots `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, and `CHANGELOG.md`, runs `release-it --dry-run`, then restores them |
| `scripts/run-web-smoke.js`   | Web smoke server wrapper     | Builds the frontend, starts `kessai-web` on port `3001`, and points it at an isolated temporary database                                           |
